    ...
    if (a == ID) {
        for (int i = 0; i < n; i++) {
            if (strcmp(ch, tab[i].lexème) == 0) {
                return i; // Return the index of the existing variable
            }
        }

        // If the variable is not found, add it to the table
        if (n >= 10) {
        ...
        strcpy(tab[n].lexème, ch);
        tab[n].type = 1;
        tab[n].address = &tab[n];
        printf("mot%s \n")
        return n++; // Return the index of the new variable and increment 'n'
    } else {
        return 0; // Return 0 if it's not an ID
    }
}
    ...
    while ( (c = fgetc(f)) != EOF ) {
        ...
        switch(etat){
            ...
            case 4:
                ...
                fseek(f, -1, SEEK_CUR);
                symbole.UL = NB;
                affect(symbole.attribute,chaine);
